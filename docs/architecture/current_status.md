...
1. Finish the broader dashboard/operator production sign-off on top of the now-signed-off live contract-chart renderer tracked in `docs/architecture/dashboard_live_chart_plan.md`, including any remaining non-chart ergonomics review.
...
- Final broader dashboard/operator production sign-off in `apps/dashboard`, now that the live contract-chart module has already cleared fresh-open browser QA, responsive overflow checks, and chart-state/operator-console sign-off
