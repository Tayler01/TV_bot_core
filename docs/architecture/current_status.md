...
The current provider-and-project integration audit now lives in `docs/architecture/integration_audit_2026-04-17.md`, with follow-up operator docs in `docs/ops/credential_setup.md`, `docs/ops/cli_standalone.md`, `docs/ops/debugging_guide.md`, and `strategies/docs/authoring.md`. The Tradovate renewal-margin follow-up from that audit is now aligned with the published 15-minute guidance, and the Databento reconnect/replay recovery regression is now in place; the main provider-level cleanup left is the final legacy GC sample removal/reclassification.
