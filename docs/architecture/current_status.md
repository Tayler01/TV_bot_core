...
- Dashboard acceptance is not met yet, but the local overview, lifecycle controls, explicit no-new-entry gating, config/env-backed runtime settings editing, strategy-library upload/validation surface, event feed, reconnect/shutdown safety controls, manual entry, persisted journal visibility, close/cancel plus open-order/fill and trade-ledger surfaces, and richer PnL/latency/health drill-downs are now wired through the host.
