...
- Phase 6 foundations are now in place, including durable trading-history record storage, live runtime/broker write-path wiring, queryable history projection through the control plane, runtime-collected latency persistence, host-level health supervision with sampled runtime-resource telemetry, and safety-critical shutdown/reconnect review control flows with explicit acceptance coverage for paper reconnect `close_position`, `leave_broker_protected`, and `reattach_bot_management` plus signal-time shutdown blocking.
...
- Targeted acceptance coverage for execution-engine no-new-entry blocking, paper-account scale-in dispatch, the paper reconnect-review decision trio through the runtime host, and signal-time shutdown review blocking
...
- Hardening pass
- Final reconnect hardening sweep inside the remaining paper-trading campaign, focused on broader restart/reconnect regression coverage beyond the current host-level acceptance set
...
- Restart/reconnect and shutdown-with-open-position acceptance are now substantially met at the host and execution layers, including explicit paper reconnect `close_position`, `leave_broker_protected`, and `reattach_bot_management` routing plus signal-time shutdown blocking coverage.
