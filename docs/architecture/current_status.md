...
- Phase 6 foundations are now in place, including durable trading-history record storage, live runtime/broker write-path wiring, queryable history projection through the control plane, runtime-collected latency persistence, and host-level health supervision.
- Phase 7 is mostly open.

V1 is not release-ready yet because the dashboard, reconnect/open-position recovery flows, richer operational surfaces, and several acceptance flows are still unfinished.
...
1. Finish the remaining safety-critical integration and paper-mode tests from `V1_ACCEPTANCE_CRITERIA.md`, especially reconnect/open-position recovery and shutdown behavior.
2. Add CPU/memory/runtime-resource sampling and richer operator-facing health/metrics views on top of the new persisted observability foundations.
...
- Shutdown-with-open-position safety flow wired end to end
