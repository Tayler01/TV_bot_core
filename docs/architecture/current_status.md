...
- Final chart-first dashboard/operator redesign and production sign-off in `apps/dashboard`, now that the live contract-chart module has already cleared fresh-open browser QA, responsive overflow checks, chart-state/operator-console sign-off, and an initial chart-centered shell with rails plus a detail dock
