...
use tv_bot_control_api::{
    ManualCommandSource, RuntimeChartConfigResponse, RuntimeChartSnapshot, RuntimeHistorySnapshot,
    RuntimeJournalSnapshot, RuntimeLifecycleCommand, RuntimeLifecycleRequest,
...
pub fn build_client() -> CliResult<Client> {
    Ok(Client::builder().timeout(Duration::from_secs(15)).build()?)
}

pub fn load_config(path: Option<&Path>, environment: &StdEnvironment) -> CliResult<AppConfig> {
...
pub async fn run_command(
    ...
    match command {
        CliCommand::Launch {
            runtime_bin,
            strategy,
        } => launch_runtime(client, base_url, runtime_bin, config_path, strategy).await?,
        CliCommand::Status => {
        ...
        CliCommand::Load { path } => {
            let response = execute_lifecycle_command(
                client,
                base_url,
        ...
        CliCommand::Warmup { command } => {
            ...
            let response = execute_lifecycle_command(client, base_url, command).await?;
            print_lifecycle_response(&response);
        }
        ...
        CliCommand::Pause => {
            let response =
                execute_lifecycle_command(client, base_url, RuntimeLifecycleCommand::Pause).await?;
            print_lifecycle_response(&response);
        }
        CliCommand::Resume => {
            let response =
                execute_lifecycle_command(client, base_url, RuntimeLifecycleCommand::Resume)
                    .await?;
            print_lifecycle_response(&response);
        }
        CliCommand::Arm {
        ...
        CliCommand::Disarm => {
            let response =
                execute_lifecycle_command(client, base_url, RuntimeLifecycleCommand::Disarm)
                    .await?;
            print_lifecycle_response(&response);
        }
        ...
        CliCommand::Console { .. } => unreachable!("console commands are handled separately"),
    }
...
async fn wait_for_runtime(client: &Client, base_url: &str) -> CliResult {
    for _ in 0..40 {
        if client
...
pub async fn fetch_status(client: &Client, base_url: &str) -> CliResult<RuntimeStatusSnapshot> {
...
pub async fn fetch_readiness(
    client: &Client,
    base_url: &str,
) -> CliResult<RuntimeReadinessSnapshot> {
    let response = client.get(format!("{base_url}/readiness")).send().await?;
    Ok(response.error_for_status()?.json().await?)
}

pub async fn fetch_history(client: &Client, base_url: &str) -> CliResult<RuntimeHistorySnapshot> {
    let response = client.get(format!("{base_url}/history")).send().await?;
    Ok(response.error_for_status()?.json().await?)
}

pub async fn fetch_journal(client: &Client, base_url: &str) -> CliResult<RuntimeJournalSnapshot> {
    let response = client.get(format!("{base_url}/journal")).send().await?;
    Ok(response.error_for_status()?.json().await?)
}

pub async fn fetch_chart_config(
    client: &Client,
    base_url: &str,
) -> CliResult<RuntimeChartConfigResponse> {
    let response = client
        .get(format!("{base_url}/chart/config"))
        .send()
        .await?;
    Ok(response.error_for_status()?.json().await?)
}

pub async fn fetch_chart_snapshot(
    client: &Client,
    base_url: &str,
    timeframe: tv_bot_core_types::Timeframe,
    limit: usize,
) -> CliResult<RuntimeChartSnapshot> {
    let response = client
        .get(format!("{base_url}/chart/snapshot"))
        .query(&[
            ("timeframe", timeframe_query_value(timeframe)),
...
pub async fn execute_lifecycle_command(
    client: &Client,
    base_url: &str,
    command: RuntimeLifecycleCommand,
) -> CliResult<RuntimeLifecycleResponse> {
    let response = client
...
fn confirm(yes: bool, prompt: &str) -> CliResult {
    if yes {
        return Ok(());
    }
...
fn resolve_runtime_binary(runtime_bin: Option<PathBuf>) -> CliResult<PathBuf> {
...
pub fn runtime_mode_label(mode: &tv_bot_core_types::RuntimeMode) -> &'static str {
    match mode {
...
pub fn arm_state_label(state: &tv_bot_core_types::ArmState) -> &'static str {
    match state {
...
pub fn warmup_status_label(status: &tv_bot_core_types::WarmupStatus) -> &'static str {
    match status {
...
pub fn format_optional_bytes(value: Option<u64>) -> String {
...
pub fn timeframe_query_value(timeframe: tv_bot_core_types::Timeframe) -> String {
    match timeframe {
        tv_bot_core_types::Timeframe::OneSecond => "1s".to_owned(),
        tv_bot_core_types::Timeframe::OneMinute => "1m".to_owned(),
        tv_bot_core_types::Timeframe::FiveMinute => "5m".to_owned(),
    }
}

pub fn timeframe_label(timeframe: tv_bot_core_types::Timeframe) -> &'static str {
    match timeframe {
        tv_bot_core_types::Timeframe::OneSecond => "1s",
        tv_bot_core_types::Timeframe::OneMinute => "1m",
        tv_bot_core_types::Timeframe::FiveMinute => "5m",
    }
}

pub fn chart_snapshot_limit(
    timeframe: tv_bot_core_types::Timeframe,
    viewport_width: usize,
) -> usize {
    fn chart_visible_bars_target(
        timeframe: tv_bot_core_types::Timeframe,
        viewport_width: usize,
    ) -> usize {
        match timeframe {
            tv_bot_core_types::Timeframe::OneSecond => {
                if viewport_width >= 1_280 {
                    150
            ...
            tv_bot_core_types::Timeframe::FiveMinute => {
                if viewport_width >= 1_280 {
                    112
                } else if viewport_width >= 900 {
                    92
            ...
            tv_bot_core_types::Timeframe::OneMinute => {
                if viewport_width >= 1_280 {
                    132
                } else if viewport_width >= 900 {
                    104
    ...
    fn chart_bars_for_two_hours(timeframe: tv_bot_core_types::Timeframe) -> usize {
        match timeframe {
            tv_bot_core_types::Timeframe::OneSecond => 7_200,
            tv_bot_core_types::Timeframe::FiveMinute => 24,
            tv_bot_core_types::Timeframe::OneMinute => 120,
        }
    }

    chart_visible_bars_target(timeframe, viewport_width).max(chart_bars_for_two_hours(timeframe))
}
