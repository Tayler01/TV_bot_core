...
- a live contract chart backed only by `/chart/config`, `/chart/snapshot`, `/chart/history`, and `/chart/stream`, with timeframe switching, fit/live-follow controls, load-older paging, active-position context, exact working-order price overlays, recent fill markers, and a working-order summary rail for the currently loaded contract
...
The remaining chart work is polish rather than first delivery: deeper degraded/reconnect-state treatment inside the chart module, final production operator ergonomics, and sign-off tracked in [docs/architecture/dashboard_live_chart_plan.md](</C:/repos/TV_bot_core/docs/architecture/dashboard_live_chart_plan.md>).
...
The remaining dashboard work is now centered on final chart/operator polish in [docs/architecture/dashboard_live_chart_plan.md](</C:/repos/TV_bot_core/docs/architecture/dashboard_live_chart_plan.md>), the last production UI sign-off items, and the final hands-on paper/demo release verification pass.
